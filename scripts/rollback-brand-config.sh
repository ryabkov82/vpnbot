...
if [[ -z "${REMOTE_TMP}" || "${REMOTE_TMP}" != /* ]]; then
  brand_err "rollback-${BRAND_LABEL}-config: invalid remote temp dir"
  exit 1
fi
...
scp -q \
  "${ENV_FILE}" \
  "${ROOT}/scripts/rollback-brand-config-remote.sh" \
...
ssh "${SERVER_USER}@${SERVER_HOST}" \
  "set -Eeuo pipefail; source $(printf %q "${REMOTE_TMP}/brand.env"); bash $(printf %q "${REMOTE_TMP}/rollback-brand-config-remote.sh")"
