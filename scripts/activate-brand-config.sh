#!/usr/bin/env bash
# Local orchestrator: build configcheck, upload, activate, smoke, cleanup.
# Requires brand profile env (use brand_profile_export or Makefile exports).
set -Eeuo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "${ROOT}"

# shellcheck source=lib/brand_ops.sh
source "${ROOT}/scripts/lib/brand_ops.sh"

SERVER_USER="${SERVER_USER:-root}"
brand_require_vars SERVER_HOST SERVICE_NAME REMOTE_DIR REMOTE_EXPLICIT_CONFIG \
  DROPIN_FILE EXPECTED_BRAND_ID BRAND_LABEL SMOKE_BASE_URL REMOTE_LEGACY_CONFIG || exit 1
brand_refresh_derived

LOCAL_TMP="$(mktemp -d)"
chmod 0700 "${LOCAL_TMP}"
REMOTE_TMP=""

cleanup() {
  local ec=$?
  if [[ -n "${REMOTE_TMP}" ]]; then
    ssh "${SERVER_USER}@${SERVER_HOST}" "rm -rf $(printf %q "${REMOTE_TMP}")" >/dev/null 2>&1 || true
  fi
  rm -rf "${LOCAL_TMP}"
  return "${ec}"
}
trap cleanup EXIT

echo "activate-${BRAND_LABEL}-config: building configcheck..."
GOOS=linux GOARCH=amd64 CGO_ENABLED=0 go build -o "${LOCAL_TMP}/configcheck" ./cmd/configcheck

echo "activate-${BRAND_LABEL}-config: preparing remote temp dir..."
REMOTE_TMP="$(ssh "${SERVER_USER}@${SERVER_HOST}" 'd=$(mktemp -d); chmod 0700 "$d"; printf %s "$d"')"
if [[ -z "${REMOTE_TMP}" || "${REMOTE_TMP}" != /* ]]; then
  brand_err "activate-${BRAND_LABEL}-config: invalid remote temp dir"
  exit 1
fi

ssh "${SERVER_USER}@${SERVER_HOST}" "test -f $(printf %q "${REMOTE_EXPLICIT_CONFIG}")"

# Export profile into a remote env file (no secrets).
ENV_FILE="${LOCAL_TMP}/brand.env"
{
  printf 'export SERVICE_NAME=%q\n' "${SERVICE_NAME}"
  printf 'export REMOTE_DIR=%q\n' "${REMOTE_DIR}"
  printf 'export REMOTE_BINARY=%q\n' "${REMOTE_BINARY}"
  printf 'export REMOTE_LEGACY_CONFIG=%q\n' "${REMOTE_LEGACY_CONFIG}"
  printf 'export REMOTE_EXPLICIT_CONFIG=%q\n' "${REMOTE_EXPLICIT_CONFIG}"
  printf 'export DROPIN_FILE=%q\n' "${DROPIN_FILE}"
  printf 'export EXPECTED_BRAND_ID=%q\n' "${EXPECTED_BRAND_ID}"
  printf 'export BRAND_LABEL=%q\n' "${BRAND_LABEL}"
} >"${ENV_FILE}"

scp -q \
  "${LOCAL_TMP}/configcheck" \
  "${ENV_FILE}" \
  "${ROOT}/scripts/activate-brand-config-remote.sh" \
  "${SERVER_USER}@${SERVER_HOST}:${REMOTE_TMP}/"
ssh "${SERVER_USER}@${SERVER_HOST}" "mkdir -p $(printf %q "${REMOTE_TMP}/lib")"
scp -q \
  "${ROOT}/scripts/lib/brand_ops.sh" \
  "${SERVER_USER}@${SERVER_HOST}:${REMOTE_TMP}/lib/"

echo "activate-${BRAND_LABEL}-config: running remote activation..."
ssh "${SERVER_USER}@${SERVER_HOST}" \
  "set -Eeuo pipefail; source $(printf %q "${REMOTE_TMP}/brand.env"); bash $(printf %q "${REMOTE_TMP}/activate-brand-config-remote.sh") $(printf %q "${REMOTE_TMP}/configcheck")"

echo "activate-${BRAND_LABEL}-config: public smoke..."
if ! SMOKE_BASE_URL="${SMOKE_BASE_URL}" BRAND_LABEL="${BRAND_LABEL}" bash "${ROOT}/scripts/smoke-brand.sh"; then
  brand_err "activate-${BRAND_LABEL}-config: smoke failed; rolling back to legacy"
  SERVER_USER="${SERVER_USER}" SERVER_HOST="${SERVER_HOST}" \
    SERVICE_NAME="${SERVICE_NAME}" REMOTE_DIR="${REMOTE_DIR}" \
    REMOTE_LEGACY_CONFIG="${REMOTE_LEGACY_CONFIG}" REMOTE_EXPLICIT_CONFIG="${REMOTE_EXPLICIT_CONFIG}" \
    DROPIN_FILE="${DROPIN_FILE}" EXPECTED_BRAND_ID="${EXPECTED_BRAND_ID}" BRAND_LABEL="${BRAND_LABEL}" \
    bash "${ROOT}/scripts/rollback-brand-config.sh"
  exit 1
fi

echo "activate-${BRAND_LABEL}-config: OK"
