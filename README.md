...
За reverse proxy нужно пробрасывать те же префиксы, что использует приложение (**`/buy`**, **`/account`**, **`/api/public/*`**, **`/api/account/*`**, включая **`/api/account/payments`**), чтобы кабинет и оплата работали через один домен с HTTPS.
