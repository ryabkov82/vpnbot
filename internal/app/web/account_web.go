...
func serveAccountSessionStart(cfg *config.Config, app accountWebApp) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		signup, err := ParseAndVerifyAccountSignupToken(secret, brandID, raw)
		if err != nil {
			writeJSONError(w, http.StatusBadRequest, "invalid_token")
			return
		}
		...
		acTok, err := CreateAccountToken(secret, brandID, normEmail, user.ID, user.Login, accountTokenTTL(cfg))
		if err != nil {
...
func serveAccountBalanceTopup(cfg *config.Config, app accountWebApp) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		paymentURL, err := payments.BuildYooKassaPaymentURL(baseURL, claims.UserID, amountRounded, time.Now().Unix(), paySystem, brandID)
		if err != nil {
