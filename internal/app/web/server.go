...
func Start(cfg *config.Config, app *service.Service, rw *remnawave.Client) {
	...
	mux.HandleFunc("/buy/", buyH)
	mux.HandleFunc("/favicon.ico", serveBrandFaviconICO(cfg))
	mux.HandleFunc("/favicon-32x32.png", serveBrandFavicon32PNG(cfg))
	mux.HandleFunc("/apple-touch-icon.png", serveBrandAppleTouchIcon(cfg))
